...
React.FC<{meals: IMeal[], mealType: string, atomRef: WritableAtom<any, any>}> = 
  ...
  return (
    <div>
      <h3 className="font-bold text-xl mb-2">{mealType}</h3>
        <div className="flex flex-col">
          {meals && meals.slice(mealSlice.from, mealSlice.to).map((main: IMeal)=> (
          <div key={main.id} onClick={()=> setCurrentMeal(main)} 
            className="my-1 border-2 border-crema-300 bg-white hover:bg-white p-2 rounded hover:border-mostaza-300 max-w-sm cursor-pointer">
            {main.name}
          </div>
          ))}
        </div>
